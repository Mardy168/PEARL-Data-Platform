...
def collect_google_news(query: str, crop: str, max_items: int = 100):
    # Two-day query buffer prevents missing articles near the rolling 24-hour boundary.
    query_with_buffer = f"({query}) when:2d"
    url = (
        "https://news.google.com/rss/search?q="
        + quote_plus(query_with_buffer)
        + "&hl=en-US&gl=US&ceid=US:en"
    )
    feed = feedparser.parse(url, request_headers={"User-Agent": "PEARL-News-Collector/3.0"})
    rows = [_base_row(e, crop, query, "Google News RSS", "Google News", url) for e in feed.entries[:max_items]]
    diagnostic = {
        "source_name": f"Google News: {crop}",
        "source_type": "Google News RSS",
        "source_url": url,
        "success": not bool(feed.bozo),
        "articles_received": len(feed.entries),
        "articles_relevant": len(rows),
        "error": str(getattr(feed, "bozo_exception", "")) if feed.bozo else "",
    }
...
def _detect_crop(text: str, crops: dict) -> str:
    ...
    low = text.lower()
    for crop in crops:
        if any(alias in low for alias in aliases.get(crop, [crop.lower()])):
            return crop
    return ""


def collect_curated_rss(source: dict, crops: dict, max_items: int = 100):
    feed = feedparser.parse(source["url"], request_headers={"User-Agent": "PEARL-News-Collector/3.0"})
    rows = []
    for entry in feed.entries[:max_items]:
        ...
        if crop:
            rows.append(
                _base_row(entry, crop, source.get("name", ""), "Curated RSS", source.get("name", ""), source["url"])
            )
    diagnostic = {
        "source_name": source.get("name", ""),
        "source_type": "Curated RSS",
